...
if __name__ == "__main__":
    ...
    # be nice and print a GPAS logo to STDOUT
    ...
    # be ready for errors!
    checks_pass = True
    errors = pandas.DataFrame(None, columns=['error', 'sample'])
    ...
    if 'fastq1' in df.columns:
        ...
        try:
            gpas_uploader_validate.NanoporeFASTQCheckSchema.validate(df, lazy=True)
        except pandera.errors.SchemaErrors as err:
            errors, checks_pass = build_errors(err, errors)
    elif 'bam' in df.columns:
        try:
            gpas_uploader_validate.BAMFASTQCheckSchema.validate(df, lazy=True)
        except pandera.errors.SchemaErrors as err:
            errors, checks_pass = build_errors(err, errors)
    ...
    if all(df.tags_ok):
        ...
        failures.rename(columns={'name': 'sample'}, inplace=True)
